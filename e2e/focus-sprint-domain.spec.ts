...
test.describe("focus sprint domain", () => {
  ...
  test("clamps elapsed and remaining time to the planned duration", () => {
    const active = { id: "focus_1", label: "Visa checklist", plannedMinutes: 15 as const, startedAt: 1_000, pausedMs: 0 };
    expect(getFocusElapsedMs(active, 1_000 + 20 * 60_000)).toBe(15 * 60_000);
    expect(getFocusRemainingMs(active, 1_000 - 1)).toBe(15 * 60_000);
  });
